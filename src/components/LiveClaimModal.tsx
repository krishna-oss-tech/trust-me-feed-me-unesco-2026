      ...
      <div className="w-full max-w-3xl surface-elevated rounded-3xl p-6 sm:p-8 border border-white/10 shadow-2xl my-6 space-y-6 relative">
        ...
        <div className="flex items-center space-x-3.5 pb-2 border-b border-white/[0.06]">
          ...
          <div>
            <div className="flex items-center space-x-2">
              ...
              <span className="px-2 py-0.5 rounded-full bg-cyan-950/80 text-cyan-300 border border-cyan-500/30 text-[10px] font-mono font-bold">
                TAVILY + AI
              </span>
            </div>
        ...
        <div className="space-y-3">
          ...
          <div className="flex flex-col sm:flex-row gap-2.5">
            <input
              ...
              maxLength={500}
              className="flex-1 px-4 py-3 rounded-xl bg-slate-900/90 border border-white/10 text-sm text-slate-100 placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-cyan-500/50 focus:border-cyan-400 transition-all font-sans"
            />

            <button
              onClick={() => handleVerify()}
              disabled={loading || !claim.trim()}
              className="px-6 py-3 rounded-xl bg-gradient-to-r from-cyan-400 to-blue-500 hover:from-cyan-300 hover:to-blue-400 disabled:opacity-50 text-slate-950 font-display font-bold text-xs sm:text-sm shadow-md shadow-cyan-500/20 flex items-center justify-center space-x-2 transition-all hover:scale-105 shrink-0 cursor-pointer"
            >
              {loading ? (
                <>
          ...
          <div className="flex flex-wrap items-center gap-1.5 pt-1">
            ...
            {SAMPLE_CLAIMS.map((sample, idx) => (
              <button
                ...
                className="text-[11px] font-mono text-slate-300 bg-slate-900/80 hover:bg-cyan-950/50 hover:text-cyan-300 px-2.5 py-1 rounded-lg border border-white/[0.08] hover:border-cyan-500/30 transition-colors text-left truncate max-w-xs cursor-pointer"
              >
                {sample}
              </button>
            ))}
        ...
        {loading && (
          <div className="p-8 rounded-2xl surface-card border border-white/[0.08] text-center space-y-3 animate-fade-in">
            ...
            <p className="text-sm font-display font-bold text-slate-200">Querying global web index via Tavily & executing MIL audit...</p>
        ...
        {result && !loading && (
          <div className="space-y-5 animate-fade-in border-t border-white/[0.06] pt-4">
            ...
            {checks[activeTab] && (
              <div className="p-4 sm:p-5 rounded-2xl surface-card border border-white/[0.08] space-y-1.5">
                ...
                <div className="space-y-2 max-h-48 overflow-y-auto pr-1">
                  {result.sources.map((src, idx) => (
                    <a
                      ...
                      className="block p-3 rounded-xl surface-card border border-white/[0.08] hover:border-cyan-500/50 hover:shadow-xs transition-all group cursor-pointer"
                    >
                      <div className="flex items-center justify-between text-xs font-bold text-cyan-400 group-hover:underline">
                        <span className="truncate max-w-md">{src.title || src.domain}</span>
                        <div className="flex items-center space-x-1 text-slate-400 group-hover:text-cyan-300 shrink-0 ml-2">
