...
Try the interactive MIL experience!`;
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/75 backdrop-blur-md animate-fade-in">
      <div className="w-full max-w-md surface-elevated rounded-2xl p-6 sm:p-7 border border-white/10 shadow-2xl relative space-y-4">
        ...
        <div className="flex items-center space-x-3">
          ...
          <div>
            ...
            <p className="text-xs text-slate-400 font-mono">Dossier summary card</p>
          </div>
        </div>

        {/* Card Summary Snippet */}
        <div className="p-5 rounded-xl surface-card border border-white/[0.08] text-xs font-mono text-slate-200 space-y-2.5">
          <div className="flex items-center space-x-1.5 text-sky-400 font-bold">
            <Award className="w-4 h-4" />
            <span className="text-sm font-display tracking-tight">{profile.archetypeTitle}</span>
          </div>
          ...
          <div className="pt-2.5 border-t border-white/[0.06] flex justify-between text-xs font-bold font-mono">
            ...
            <span>Accuracy: <strong className="text-sky-400">{profile.metrics.accuracyScore}%</strong></span>
          </div>
        </div>

        <button
          onClick={handleCopy}
          className="w-full py-3.5 rounded-xl bg-gradient-to-r from-sky-400 to-blue-500 hover:brightness-110 active:scale-[0.97] text-slate-950 font-display font-bold text-xs sm:text-sm shadow-md shadow-sky-500/20 flex items-center justify-center space-x-2 transition-all cursor-pointer"
        >
          {copied ? (
            <>
