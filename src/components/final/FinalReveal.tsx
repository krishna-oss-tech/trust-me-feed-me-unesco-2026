...
import { RotateCcw, Share2, BookOpen, ShieldCheck, Globe } from 'lucide-react';
import { ScrambleText } from '../ScrambleText';

export const FinalReveal: React.FC = () => {
  const { restartFromBeginning, profile } = useGame();
  ...
  return (
    <div className="w-full max-w-3xl mx-auto space-y-10 animate-fade-in text-center py-8 px-4 relative z-10">
      ...
      {/* High-Impact Emotional Reveal */}
      <div className="space-y-6">
        
        <div className="inline-flex items-center space-x-2 px-4 py-1.5 rounded-full bg-sky-950/40 border border-sky-500/25 text-sky-300 text-xs font-mono font-semibold shadow-xs">
        ...
        <div className="space-y-4">
          ...
          <p className="text-base sm:text-xl font-display font-medium text-slate-400">
            With the choices you made in the last two minutes.
          </p>

          <div className="py-6 my-4 border-y border-white/[0.08]">
            <h2 className="text-2xl sm:text-4xl font-display font-bold text-sky-300">
              &ldquo;<ScrambleText text="What you trust shapes what you see." duration={1400} />&rdquo;
            </h2>
          </div>
        </div>
      ...
      <div className="flex flex-col sm:flex-row items-center justify-center gap-3.5 max-w-lg mx-auto">
        ...
        <button
          ...
          className="w-full sm:w-auto px-6 py-3.5 rounded-xl surface-elevated hover:bg-slate-800 border border-white/10 text-slate-200 font-display font-bold text-xs sm:text-sm shadow-xs flex items-center justify-center space-x-2 transition-all active:scale-[0.97] cursor-pointer"
        >
        ...
        <button
          ...
          className="w-full sm:w-auto px-7 py-3.5 rounded-xl bg-gradient-to-r from-sky-400 to-blue-500 hover:brightness-110 active:scale-[0.97] text-slate-950 font-display font-bold text-xs sm:text-sm shadow-lg shadow-sky-500/20 flex items-center justify-center space-x-2 transition-all cursor-pointer"
        >
        ...
        <button
          ...
          className="w-full sm:w-auto px-6 py-3.5 rounded-xl surface-elevated hover:bg-slate-800 border border-white/10 text-slate-300 font-display font-bold text-xs sm:text-sm shadow-xs flex items-center justify-center space-x-2 transition-all active:scale-[0.97] cursor-pointer"
        >
      ...
      {/* Accordion UNESCO MIL Framework Guide */}
      {showLearnMore && (
        <div className="surface-elevated rounded-2xl p-6 sm:p-8 border border-white/[0.08] text-left space-y-5 animate-fade-in shadow-xl">
